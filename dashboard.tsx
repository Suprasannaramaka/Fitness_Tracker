...
import { getMotivationalMessage } from "../FitTrack_Assets/assets/assets";
import type { FoodEntry } from "../FitTrack_Assets/types";
import type { ActivityEntry } from "../FitTrack_Assets/types";
import Card from "../components/ui/Card";
import CaloriesChart from "../FitTrack_Assets/assets/CaloriesChart";
import ProgressBar from "../components/ui/ProgressBar";
import { FlameIcon, HamburgerIcon , ScaleIcon, TrendingUpIcon, ZapIcon  , Ruler} from "lucide-react";
import { Activity } from "lucide-react";
const Dashboard = () => {
  const {user,allActivityLogs,allFoodLogs} = useAppContext()
  const DAILY_CALORIE_LIMIT : number = user?.dailyCalorieIntake || 2000;
  const today = new Date().toISOString().split("T")[0];
  const todayFood = allFoodLogs.filter((f: FoodEntry) => f.createdAt?.split("T")[0] === today)
      ...
      <div className="dashboard-grid">
        ...
        <Card className="shadow-lg col-span-2">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-xl bg-orange-100 flex items-center justify-center">
                <HamburgerIcon className="w-6 h-6 text-orange-600" />
              </div>
              <div>
          ...
          <div className="mt-4 flex justify-between items-center">
            <div
              className={`px-3 py-1.5 rounded-lg ${
                remainingCalories >= 0
                  ? "bg-emerald-50 dark:bg-emerlad-900/10 text-emerald-700 dark:text-emerald-400"
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-right">
              <p className="text-sm text-slate-500 dark:text-slate-400">Daily Burn Goal</p>
        ...
        {/* Stats Row*/}
        <div className="dashbaord card-grid">
          {/*Active Minutes*/}
        <Card>
         ...
         <p>Active</p>
          ...
          <p className="text-sm text-slate-800">minutes today</p>
        </Card>
         {/*Activities  Count*/}
        <Card>
          <div className="flex items-center gap-3 mb-3">
         ...
         <p>Workouts</p>
          ...
          <p className="text-sm text-slate-800">Activites logged</p>
        </Card>
        </div>
         {/*Goal Card*/} 
         {user && ((
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="w-12 h-12 rounded-xl bg-white/10 flex items-center justify-center">
            ...
            <p>{user.goal === 'maintain' && 'maintain Weight'}</p>
          ...
          {user && user.weight && (
            <Card>
              ...
              <div className="w-12 h-12 rounded-xl bg-indigo/10 flex items-center justify-center">
            ...
            <ScaleIcon className="w-6 h-6 text-indigo-500"/>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold text-slate-700 dark:text-slate-200">{user.weight}</span>
                  </div>
                </div>
                {user.height && (
                   <div className="flex justify-between items-center">
         ...
         <Card>
          <h3 className="">This Week's Progress</h3>
          <CaloriesChart />
         </Card>
      </div>
    </div>
